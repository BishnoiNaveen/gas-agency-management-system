const GAMS = (() => {
  ...
  const defaultData = () => ({
    ...
    complaints: [],
    payments: [],
    settings: { adminPassword: null, adminRecoveryCode: null },
  ...
  async function adminLogin() {
    ...
    try {
      ...
      errEl.textContent = e.message || `Invalid credentials. Attempt ${loginAttempts} of 3`;
      errEl.classList.remove('hidden');
    }
  }
  ...
  async function customerLogin() {
    ...
    try {
      ...
      errEl.textContent = e.message || `Invalid username or password. Attempt ${custLoginAttempts} of 3`;
      errEl.classList.remove('hidden');
    }
  }
